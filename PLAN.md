...
### Phase 3: Signal generation (~1 day): ✅ done 2026-09-11 (prompt v1 frozen unchanged, tag `prompt-frozen`)
...
### Phase 4: Test run and report (~0.5 day): ✅ done 2026-09-11 (run once, $0.0062)
...
### Phase 5: README and definition of done (~0.5 day): ✅ done 2026-09-11
...
- [x] `pip install -r requirements.txt` then `make run` regenerates every figure and number in the README, verified by the fresh-clone `--offline` diff (2026-09-11: fresh clone + new venv + `make reproduce` without a key left `git status` empty)
- [x] `pytest` green and `ruff check .` clean, locally and in CI
- [x] The README follows spec §8 order, and the limitations section is present
- [x] Every headline metric appears next to its baseline and n (accuracy vs random vs always-up, Brier vs 0.25, ECE vs null)
- [x] The coverage funnel is reported, with parse failures counted rather than dropped
- [x] The no-look-ahead rule is documented in the `labels.py` docstring and the README, and enforced by the invariant test
- [x] The cost cap is enforced in code, and actual spend for a full run is recorded in `metrics.json` (actual: $0.0089 for the whole project, 150 calls)
- [x] API keys come from the environment only, and `.env` is gitignored
- [x] No real LLM calls in tests or CI
